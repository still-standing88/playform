...
class ExplorerView(QListWidget):
    ...
    def __init__(self, explorer:Explorer, player:mpv_video_player.MPVVideoPlayer, parent, **kw):
        self._callbacks = kw
        ...
        self.itemClicked.connect(self.forward)
        self.itemActivated.connect(self.forward)
    ...
    def set_shortcuts(self):
        hotkeys = key_config.key_dict["Explorer"]
    ...
    def update_path(self):
        self._execute_callback("update_path", self._explorer.current_path)
    ...
    def set_item_info(self):
        ...
        info = f"Type extension: {item_info.ext}\rDate modified: {item_info.modify_date}{"\rsize: " + item_info.size if item_info.type == PathType.FILE else ""}" # type: ignore
...
#        self.tooltip = tooltip(self.info,position,3,self,self.parent)
    ...
    def _execute_callback(self, callback_name:str, param:str):
    ...
    def onItemChange(self, c, p):
        ...
        self._focused_item_path = f"{self._explorer.current_path}/{item_name}"
        self.set_item_info()

        if item_info.type == PathType.FILE and self._focused_item_path != self._current_media: # type: ignore
            self._current_media = self._focused_item_path

            if self._instance is None:
                self._instance = self._player.create_file_instance(self._focused_item_path)
            else:
                self._instance.load_file(self._focused_item_path)
            self._instance.stop()
