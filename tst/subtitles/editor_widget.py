from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGroupBox,
                             ...
                             QTabWidget, QRadioButton, QDoubleSpinBox, 
...
class EditorWidget(QWidget):
    ...
    def create_timing_tab(self):
        ...
        self.from_fps_spin.setRange(0.01, 1000.0); self.to_fps_spin.setRange(0.01, 1000.0)
        self.from_fps_spin.setValue(23.976); self.to_fps_spin.setValue(25.000)
    ...
    def create_text_tab(self):
        ...
        op_group = QGroupBox("Text Operation"); op_layout = QHBoxLayout()
        ...
        op_layout.addWidget(self.op_none_radio); op_layout.addWidget(self.op_find_replace_radio)
        op_layout.addWidget(self.op_prefix_suffix_radio); op_layout.addWidget(self.op_change_case_radio)
        op_group.setLayout(op_layout)
        self.op_none_radio.setChecked(True)
        self.op_stack = QStackedWidget()
        self.op_stack.addWidget(QWidget())
        fr_widget = QWidget(); fr_layout = QFormLayout(fr_widget)
        self.find_edit = QLineEdit(); self.replace_edit = QLineEdit()
        self.case_sensitive_check = QCheckBox("Case Sensitive")
        fr_layout.addRow("Find:", self.find_edit); fr_layout.addRow("Replace With:", self.replace_edit)
        fr_layout.addRow(self.case_sensitive_check)
        self.op_stack.addWidget(fr_widget)
        ps_widget = QWidget(); ps_layout = QFormLayout(ps_widget)
        self.prefix_edit = QLineEdit(); self.suffix_edit = QLineEdit()
        ps_layout.addRow("Prefix:", self.prefix_edit); ps_layout.addRow("Suffix:", self.suffix_edit)
        self.op_stack.addWidget(ps_widget)
        cc_widget = QWidget(); cc_layout = QFormLayout(cc_widget)
    ...
    def add_file(self):
        ...
        if file:
            ...
            detected_encoding = subtitle_handler.detect_encoding(file)
    ...
    def load_file_for_preview(self, current_item, previous_item):
        ...
        events = subtitle_handler.read_subtitle_file(self.file_path, encoding=encoding)
        ...
        self.update_preview_with_text_edits()
        self.status_label.setText("Ready")
    ...
    def update_preview_with_text_edits(self):
        ...
        preview_events = []
        op = "None"
        if self.op_find_replace_radio.isChecked(): op = "Find and Replace"
        elif self.op_prefix_suffix_radio.isChecked(): op = "Add Prefix/Suffix"
        elif self.op_change_case_radio.isChecked(): op = "Change Case"

        for event in self.original_events:
            new_event = event.copy()
            original_text = new_event["text"]
            ...
            if op == "Find and Replace":
                ...
                modified_text = f"{self.prefix_edit.text()}{original_text}{self.suffix_edit.text()}"
            elif op == "Change Case":
                style = self.case_style_combo.currentText()
                if style == "UPPERCASE": modified_text = original_text.upper()
                elif style == "lowercase": modified_text = original_text.lower()
                elif style == "Title Case": modified_text = original_text.title()
            
            new_event["text"] = modified_text
            preview_events.append(new_event)

        self.preview_table.setRowCount(len(preview_events))
        for row, event in enumerate(preview_events):
    ...
    def run_processing(self):
        ...
        text_op = "None"
        if self.op_find_replace_radio.isChecked(): text_op = "Find and Replace"
        elif self.op_prefix_suffix_radio.isChecked(): text_op = "Add Prefix/Suffix"
        elif self.op_change_case_radio.isChecked(): text_op = "Change Case"

        options = {
            "input_encoding": self.input_encoding_combo.currentText(), "shift_time": self.shift_edit.text(),
            "transform_framerate": self.transform_fps_check.isChecked(),
            "from_fps": self.from_fps_spin.value(), "to_fps": self.to_fps_spin.value(),
            "text_operation": text_op, "find_text": self.find_edit.text(),
            "replace_text": self.replace_edit.text(),
            "case_sensitive": self.case_sensitive_check.isChecked(),
            "prefix": self.prefix_edit.text(), "suffix": self.suffix_edit.text(),
            "case_style": self.case_style_combo.currentText(),
        }
        
        self.status_label.setText("Processing...")
        result = subtitle_handler.process_subtitles([self.file_path], output_dir, options)
