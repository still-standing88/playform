from PySide6.QtWidgets import (
    ...
    QLineEdit, QPushButton, QLabel, QTreeWidget, QTreeWidgetItem, QTextEdit,
    QMenu, QMessageBox, QInputDialog, QProgressDialog, QDialog, QTextBrowser,
...
class EntryDetailDialog(QDialog):
    ...
    def display_entry(self):
        ...
        if isinstance(self.entry, dict):
            attrs = self.entry
        elif hasattr(self.entry, 'keys') and callable(self.entry.keys):
            try:
        ...
        if not attrs and hasattr(self.entry, '__dict__'):
    ...
    def format_value_as_text(self, value):
        """Format value as plain text for clipboard"""
        if value is None:
            return "None"
        ...
        if isinstance(value, (list, tuple)):
            if not value:
                return "Empty"
            items = [str(item) for item in value[:50]]  # More items for text
            if len(value) > 50:
...
class FeedWidget(QWidget):
    # Common fields to display in tree columns
    COMMON_FIELDS = ['title', 'published', 'link']
    # Fields with long text that go in text edit
    ...
    BLACKLIST = ['published_parsed', 'updated_parsed', 'guidislink', 
                 ...
                 'links', 'content']  # content is handled specially
    ...
    def setup_ui(self):
        ...
        # Main splitter: Feed List | Entry Tree + Details
        ...
        # Detail text area for long content
    ...
    def get_entry_value(self, entry, field):
        """Get value from entry, handling both dict and object access"""
        val = None
        ...
        # Try alternate access if needed
    ...
    def format_date(self, entry):
        ...
        # If it's a parsed tuple, convert to string
    ...
    def display_entry_detail(self, entry):
        """Display long text fields in the detail text browser with sanitized HTML"""
        html_parts = []
        html_parts.append("<div style='font-family: Arial, sans-serif; padding: 10px; line-height: 1.6;'>")
        
        # Get summary/description/content
        ...
        if len(html_parts) > 2:  # More than just opening and closing tags
    ...
    def show_full_entry(self, item):
        """Show dialog with complete entry dump"""
    ...
    def copy_direct_media_link(self, item):
        """Copy direct media link (MP3, etc.) to clipboard"""
        entry = item.data(0, Qt.ItemDataRole.UserRole)
        if not entry:
            return
    ...
    def select_next_entry(self):
        ...
        if current:
            ...
            if index < self.entry_tree.topLevelItemCount() - 1:
                self.entry_tree.setCurrentItem(self.entry_tree.topLevelItem(index + 1))
    ...
    def select_page_down(self):
        ...
        if current:
            ...
            self.entry_tree.setCurrentItem(self.entry_tree.topLevelItem(new_index))
    ...
    def select_last_entry(self):
        ...
        if count > 0:
            self.entry_tree.setCurrentItem(self.entry_tree.topLevelItem(count - 1))