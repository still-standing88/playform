...
class MPVMediaInterface(AVMediaInterface):
    ...
    def play(self, id: int):
        ...
        # transition) instead of just letting it finish. Only consumed once,
...
class MPVVideoPlayer(AVPlayer):
    ...
    def init(self, *args, **kw):
        ...
        # Additional, independent listener -- set_start_file_callback
        # appends rather than replaces, so this doesn't disturb the app's
        # own file-loaded listener (resume-last-position) registered
        # separately in player_init.py.
        self.set_start_file_callback(lambda event: self._on_track_file_loaded())
