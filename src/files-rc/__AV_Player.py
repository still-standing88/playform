...
class AVPlayer(ABC):
    ...
    def load_playlist(self, playlist:Playlist, auto_play:bool = False):
        if playlist is not None:
            ...
            if self.playlist_shuffle_mode == AVPlaylistShuffleMode.SHUFFLE:
    ...
    def _monitor_playback(self):
        # Ensure COM is initialized on this worker thread (Windows only)
        _win_com = None
        if os.name == "nt":
            try:
                ...
                try:
                    if self._primary_instance:
                        ...
                        if hasattr(self._controler, 'is_heavy_usage_period') and self._controler.is_heavy_usage_period():
                            sleep_time = heavy_usage_sleep_time
