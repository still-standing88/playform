...
from PySide6.QtCore import (
    ...
    Signal, QSortFilterProxyModel, QItemSelectionModel, QMimeData
)
...
class ListModel(QAbstractListModel):
    ...
    def mimeTypes(self) -> List[str]:
        return ["application/x-playform-listctrl-rows"]

    def mimeData(self, indexes) -> QMimeData:
        mime_data = QMimeData()
        rows = sorted({index.row() for index in indexes if index.isValid()})
        row_ids = [
            str(self._items[row].row_id)
            for row in rows
            if 0 <= row < len(self._items)
        ]
        mime_data.setData(
            "application/x-playform-listctrl-rows",
    ...
    def dropMimeData(self, data, action, row, column, parent) -> bool:
        if action != Qt.DropAction.MoveAction:
            return False
        if not data.hasFormat("application/x-playform-listctrl-rows"):
            return False

        raw_ids = bytes(data.data("application/x-playform-listctrl-rows")).decode("utf-8")
        if not raw_ids:
            return False

        requested_ids = [int(value) for value in raw_ids.split(",") if value]
        source_rows = [
            index
            for index, item in enumerate(self._items)
            if item.row_id in requested_ids
        ]
        if not source_rows:
            return False

        if row < 0:
            row = parent.row() if parent.isValid() else len(self._items)
        row = max(0, min(row, len(self._items)))

        moved_items = [self._items[index] for index in source_rows]

        self.beginResetModel()
        for index in reversed(source_rows):
            del self._items[index]

        adjusted_row = row - sum(1 for index in source_rows if index < row)
        for offset, item in enumerate(moved_items):
            self._items.insert(adjusted_row + offset, item)
        self.endResetModel()
        return True
