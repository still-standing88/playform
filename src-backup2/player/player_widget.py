...
class PlayerWidget(QWidget):
    # Duplicate connect_signals method removed. Only one definition remains.
    ...
    def _on_resolution_changed(self, resolution):
        # resolution is a tuple (width, height) or None for original
        if resolution:
    ...
    def __init__(self, parent=None):
        ...
        self._session_filters = {}  # Persist filters for session

        self.setup_ui()
        self.layout_widgets()

        self.player = av_play.MPVVideoPlayer()
    ...
    def _init_player(self):
        yt_dlp_path = os.path.join(get_app_path(), "bin", "yt-dlp.exe" if os.name == 'nt' else "yt-dlp")
        try:
    ...
    def _on_forward_clicked(self):
        if self.player: self.player.forward(10)
        
    def _on_backward_clicked(self):
        if self.player: self.player.backward(10)
    ...
    def _on_seek_changed(self, position):
        if self.player.primary_instance is not None: self.player_controls.set_time_text(f"{format_time(position)} / {format_time(self.player.primary_instance.get_length())}")
    ...
    def _on_jump_to_end(self):
        ...
        if instance:
            try:
                ...
                instance.set_position(max(0, length - 5))
            except av_play.AVError as e:
                msg = f"Seek error: {getattr(e, 'message', str(e))}"
                QMessageBox.critical(self, "Seek Error", msg)
        ...
        try:
            if activated:
                if filter_name in self.available_filters and filter_name not in self._active_filters:
                    ...
                    self._session_filters[filter_name] = gui_params.copy()  # Save params for session
            else:
    ...
    def load_playlist(self, playlist: av_play.Playlist, start_index: int = 0):
        try:
            # Release previous instance before switching playlist
            ...
            self.player.load_playlist(playlist, auto_play=True)
            # Reapply session filters to new instance
