...
class PlayerControls(QWidget):
    ...
    def setup_ui(self):
        ...
        self.resolution_combo.addItems(["Original", "480p", "720p", "1080p"])
        self.resolution_combo.setCurrentIndex(0)
        self.resolution_combo.setToolTip("Resolution/Scale")
    ...
    def connect_signals(self):
        # ...existing code...
        self.fullscreen_btn.clicked.connect(self._emit_fullscreen)
    ...
    def _emit_fullscreen(self):
        ...
        current = self.fullscreen_btn.property("fullscreen")
        enabled = not bool(current) if current is not None else True
        self.fullscreen_btn.setProperty("fullscreen", enabled)
        self.fullscreenToggled.emit(enabled)
        self.fullscreen_btn.setText("⛶" if not enabled else "🗗")

    def _emit_speed(self, idx):
        speeds = [0.5, 1.0, 1.25, 1.5, 2.0]
        self.speedChanged.emit(speeds[idx])

    def _emit_resolution(self, idx):
        resolutions = [None, (854,480), (1280,720), (1920,1080)]
        self.resolutionChanged.emit(resolutions[idx])
